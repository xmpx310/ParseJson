...
class XHProfRuns_Default implements iXHProfRuns {
  ...
  function list_runs($order=0) {
    if (is_dir($this->dir)) {
        echo "<hr/>Existing runs:\n<ul>\n";
        $files = glob("{$this->dir}/*.{$this->suffix}");
        if($order == 1){            
            function getWallTime($file){
                $file_component = explode('-', basename($file));
                array_pop($file_component);
                $wallTime = array_pop($file_component);
                return floatval($wallTime);
            }
            usort($files, create_function('$a,$b', 'return getWallTime($b) - getWallTime($a);'));
        ...
        foreach ($files as $file) {
            ...
            $file_component = explode('.', basename($file));
            array_pop($file_component);
            $source = array_pop($file_component);
            $run = implode('.',$file_component);
            $run_component = explode('-',$run);
            array_pop($run_component);
            // array_pop($run_component);
            $displayName = implode('-',$run_component);
            echo '<li><a href="' . htmlentities($_SERVER['SCRIPT_NAME'])
                . '?run=' . htmlentities($run) . '&source='
                . htmlentities($source) . '&date='.date("Y-m-d", filemtime($file)).'">'
                . htmlentities($displayName) . "</a><small> "
